# Simulation for Figure 14 of the appendix.
...
def two_matrices_non_diag_two(folder, seed=SEED):
    d_list = range(10, 2001, 4)
    angle_list = jnp.linspace(0, jnp.pi/4, 10)
    group_sizes = [100, 100]
    ...
    for angle in angle_list:
        risks_list = []
        for d in d_list:
            logging.log(logging.INFO, f'Processing angle={angle}, d={d}...')
            sqrt_dSig_list = [
                jnp.diag(jnp.concatenate([jnp.ones(round(d/2)), -1 * jnp.ones(d - round(d/2))])),
                jnp.concatenate(
                    [
                        jnp.concatenate([jnp.cos(angle) * jnp.diag(jnp.ones(round(d/2))), jnp.sin(angle) * jnp.diag(jnp.ones(d - round(d/2)))], axis=1),
                        jnp.concatenate([jnp.sin(angle) * jnp.diag(jnp.ones(round(d/2))), jnp.cos(angle) * jnp.diag(jnp.ones(d - round(d/2)))], axis=1)
                    ],
                    axis=0
                ),
            ]
...
if __name__ == "__main__":
    ...
    two_matrices_non_diag_two(HERE / "log", seed=args.seed)
