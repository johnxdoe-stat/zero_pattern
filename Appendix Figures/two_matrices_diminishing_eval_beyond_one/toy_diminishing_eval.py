# Simulation for Figure 10 of the appendix.
...
def two_matrices_diminishing_eval_beyond_one(folder, seed=SEED):
    ...
    for eval_size in eval_size_list:
        ...
        for d in d_list:
            ...
            sqrt_dSig_list = [
                jnp.diag(jnp.concatenate([jnp.ones(round(d/2)) / eval_size, jnp.ones(d - round(d/2))])),
                jnp.diag(jnp.concatenate([jnp.ones(d - round(d/4)), jnp.ones(round(d/4)) / eval_size])),
            ]
...
if __name__ == "__main__":
    ...
    two_matrices_diminishing_eval_beyond_one(HERE / "log", seed=args.seed)
