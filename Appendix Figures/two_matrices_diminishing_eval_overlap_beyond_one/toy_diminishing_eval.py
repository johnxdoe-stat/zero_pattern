# Simulation for Figure 12 of the appendix.
...
def two_matrices_diminishing_eval_overlap_beyond_one(folder, seed=SEED):
    d_list = range(10, 2001, 5)
    eval_size_list = [1,1e1,1e2,1e3,1e4]
    group_sizes = [100, 100]
    ...
    for eval_size in eval_size_list:
        risks_list = []
        for d in d_list:
            logging.log(logging.INFO, f'Processing eval_size={eval_size}, d={d}...')
            sqrt_dSig_list = [
                jnp.diag(jnp.concatenate([jnp.ones(round(d/3)) / eval_size, jnp.ones(d - round(d/3)) ])),
                jnp.diag(jnp.concatenate([jnp.ones(d - round(3*d/4)), jnp.ones(round(3*d/4)) / eval_size])),
            ]
...
if __name__ == "__main__":
    ...
    two_matrices_diminishing_eval_overlap_beyond_one(HERE / "log", seed=args.seed)
