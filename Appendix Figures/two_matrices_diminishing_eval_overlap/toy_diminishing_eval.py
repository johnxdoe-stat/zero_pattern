# Simulation for Figure 11 of the appendix.
...
def two_matrices_diminishing_eval_overlap(folder, seed=SEED):
    d_list = range(10, 2001, 5)
    eval_size_list = [0, 1e-4, 1e-3, 1e-2, 1]
    group_sizes = [100, 100]
    ...
    for eval_size in eval_size_list:
        ...
        for d in d_list:
            ...
            sqrt_dSig_list = [
                jnp.diag(jnp.concatenate([jnp.ones(round(d/3)), jnp.ones(d - round(d/3)) * eval_size ])),
                jnp.diag(jnp.concatenate([jnp.ones(d - round(3*d/4)) * eval_size, jnp.ones(round(3*d/4))])),
            ]
...
if __name__ == "__main__":
    ...
    two_matrices_diminishing_eval_overlap(HERE / "log", seed=args.seed)
