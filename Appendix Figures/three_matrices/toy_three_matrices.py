# Simulation for Figure 8 of the appendix.
...
def three_matrices(folder, seed=SEED):
    d_list = range(10, 2001, 5)
    signal_pos_list = [1/12, 2/12, 3/12, 4/12, 5/12, 6/12, 7/12]
    ...
    for signal_pos in signal_pos_list:
        risks_list = []
        for d in d_list:
            logging.log(logging.INFO, f'Processing signal_pos={signal_pos}, d={d}...')
            signal_start = round(d * signal_pos)
            sqrt_dSig_list = [
                jnp.diag(jnp.concatenate([jnp.ones(round(d/3)), jnp.zeros(d - round(d/3))])),
                jnp.diag(jnp.concatenate([jnp.zeros(d - round(d/2)), jnp.ones(round(d/2))])),
                jnp.diag(jnp.concatenate([jnp.zeros(signal_start), jnp.ones(round(d/6)), jnp.zeros(d - signal_start - round(d/6))])),
            ]
    ...
    np.savez(f'{folder}/output',
             d_list=d_list,
             signal_pos_list=signal_pos_list,
...
if __name__ == "__main__":
    ...
    three_matrices(HERE / "log", seed=args.seed)
