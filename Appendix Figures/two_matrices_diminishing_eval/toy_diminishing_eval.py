# Simulation for Figure 9 of the appendix.
...
def two_matrices_diminishing_eval(folder, seed=SEED):
    ...
    for eval_size in eval_size_list:
        ...
        for d in d_list:
            ...
            sqrt_dSig_list = [
                jnp.diag(jnp.concatenate([jnp.ones(round(d/2)), jnp.ones(d - round(d/2)) * eval_size ])),
                jnp.diag(jnp.concatenate([jnp.ones(d - round(d/4)) * eval_size, jnp.ones(round(d/4))])),
            ]
...
if __name__ == "__main__":
    ...
    two_matrices_diminishing_eval(HERE / "log", seed=args.seed)
